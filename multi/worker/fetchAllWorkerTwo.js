const fetchDistancesWorkerTwo = new Worker('../worker/fetchDistancesWorkerTwo.js');
self.addEventListener('message', function(e) {
    ...
    Promise.all(urls.map(url => fetch(url).then(response => response.json())))
      .then(data => {
        ...
        let newData = {cars: [], prices: {'priceOne': [], 'priceTwo': []}, distances: {'distanceOne': 0, 'distanceTwo': 0}};
        ...
        fetchDistancesWorkerTwo.postMessage(latLngs);
        new Promise((resolve, reject) => {
          fetchDistancesWorkerTwo.onmessage = function(e) {
            if (e.data.error) {
              reject(e.data.error);
            } else {
              resolve(e.data);
